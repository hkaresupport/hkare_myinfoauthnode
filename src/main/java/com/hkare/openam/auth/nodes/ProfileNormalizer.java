...
/**
 * User profile normaliser. It uses configured Account Mapper and Attribute
 * Mappers to normalise the user attributes.
 */
@Singleton
public class ProfileNormalizer {
	 ...
	 * @throws AuthLoginException when error occurs while instantiating the provider
	 *                            class.
	 */
	public AccountProvider getAccountProvider(MyInfoAuthNode.Config config) throws AuthLoginException {
	 ...
	 * @throws AuthLoginException when error occurs while instantiating the mapper
	 *                            class.
	 */
	public AttributeMapper getAccountMapper(MyInfoAuthNode.Config config) throws AuthLoginException {
	...
	/**
	 * Gets normalized attributes for the Account lookup. It internally uses Account
	...
	public Map<String, Set<String>> getNormalisedAccountAttributes(UserInfo userInfo, JwtClaimsSet jwtClaimsSet,
			MyInfoAuthNode.Config config) throws AuthLoginException {
		try {
	...
	/**
	 * Gets normalized attributes for the user. It internally uses Attribute Mapper
	 * configurations.
	 *
	 * @param userInfo     The user attributes from the social provider.
	 * @param jwtClaimsSet The id token claim from the social provider.
	...
	public Map<String, Set<String>> getNormalisedAttributes(UserInfo userInfo, JwtClaimsSet jwtClaimsSet,
		...
		try {
			return unmodifiableMap(OAuthUtil.getAttributesMap(config.cfgAttributeMappingConfiguration(),
					config.cfgAttributeMappingClasses(), userInfo.getRawProfile().toString(), jwtClaimsSet));
		} catch (OAuthException e) {
