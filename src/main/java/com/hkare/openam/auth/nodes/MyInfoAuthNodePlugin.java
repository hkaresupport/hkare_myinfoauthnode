 ...
 * SMS service on install, you can add the following setter:
 * 
 * <pre>
 * <code>
 * {@code @Inject}
 ...
 * example. PluginTools javadoc may be found <a href=
 ...
 * the instances on which {@link #onAmUpgrade(String, String)} will be called.
 * Guice-injected properties will also <i>not</i> be populated during that
 * method call.
 * </p>
 * <p>
 * Plugins should <i>not</i> use the
 * {@code ShutdownManager}/{@code ShutdownListener} API for handling shutdown,
 * as the order of calling those listeners is not deterministic. The
 * {@link #onShutdown()} method for all plugins will be called in the reverse
 * order from the order that {@link #onStartup()} was called, with dependent
 * plugins being notified after their dependencies for startup, and before them
 * for shutdown.
 * </p>
 * 
 * @supported.all.api
 * @since AM 5.5.0
 */
public class MyInfoAuthNodePlugin extends AbstractNodeAmPlugin {
	...
	protected Map<String, Iterable<? extends Class<? extends Node>>> getNodesByVersion() {
		logger.debug("*******Initialize the plugin node classes*********");
		return ImmutableMap.of(MyInfoAuthNodePlugin.currentVersion, asList(MyInfoAuthNode.class));
	}
	...
	public String getPluginVersion() {
		return MyInfoAuthNodePlugin.currentVersion;
	}
}
